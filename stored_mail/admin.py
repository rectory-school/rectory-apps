...
class OutgoingMailAdminSentStatus(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == 'discarded':
            return queryset.filter(sent_at__isnull=True, discard_after__lte=timezone.now())

        if self.value() == 'unsent':
            return queryset.filter(sent_at__isnull=True, discard_after__gt=timezone.now())
...
class OutgoingMailAdmin(ViewOnlyAdminMixin, admin.ModelAdmin):
    ...
    fields = ['from_name', 'from_address', 'subject', 'created_at',
              'discard_after', 'last_send_attempt', 'sent_at', 'encoded']
